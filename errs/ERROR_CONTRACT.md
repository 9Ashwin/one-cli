...
4. `Code` is the upstream numeric code when known (e.g. platform API code).
...
```json
{
  ...
  "error": {
    ...
    "code": 12345,
    "message": "missing scope `pets:create` for app cli_xxx",
    "hint": "run onecli auth login --scope pets:create",
    "log_id": "20260702-0a1b2c3d",
    "missing_scopes": ["pets:create"],
    "console_url": "https://example.com/app/cli_xxx/auth?q=..."
  }
}
```
...
| `api` | server-side platform error w/o specific bucket | 1 | `APIError` |
...
can write `if cmd; then ... fi`. `onecli auth check` is the canonical
...
A batch command that processes many items can finish in a third state,
neither full success nor a single error: some items succeeded and some
failed. Its primary output is the per-item result, so it does **not**
belong in a `stderr` error envelope.
...
The canonical producer surface is the **builder API in `errs/types.go`** (per
type: struct + `NewXxxError` + chained `WithX` setters live in one place):
...
| Token lacks scope | framework classifier |
...
| Platform API error | framework classifier |
...
```go
return fmt.Errorf("calling /api: %v", err)  // %v strips the typed shape
...
maps `Category` to the shell code. A new exit-code requirement means a new
`Category`, not a one-off override at the call site.
...
| `Hint` | What to do next | Imperative ("run `onecli auth login`", "use `--as user`") |
...
return errs.NewNetworkError(errs.SubtypeNetworkTransport,
    "request to /api failed after 3 retries").
...
the framework classifier still uses them on the dynamic dispatch path where
a `Problem` value is composed once and wrapped per Category branch. Outside
that pattern, new code should reach for the builder.
...
the typed `*errs.ValidationError`; the helper, not `Execute`, sets `Param`.
...
Prefer the `Cause` field over `fmt.Errorf("...: %w", err)` when
...
If you find yourself reaching for `WrapDoAPIError` from a domain
...
1. Add a constant in `errs/subtypes.go` under the right Category block.
   Subtypes are framework-shared — platform-specific Subtypes are an
   ...
   `quota_exceeded`).
2. If it maps from a platform code, register the mapping in the
   framework classifier.
3. Add a dispatch test in the classifier test suite.
...
specific category.** The platform code is unknown to the classifier and
fell through to the generic bucket. Add the code to the framework
classifier with the right Category and Subtype, plus a dispatch test.

**Envelope shows `type=internal subtype=sdk_error`.** Origin is the SDK
boundary taking the non-transport branch. Check: did the SDK fail to decode
the response (look for `subtype=invalid_response` in the wrapped chain)? Was
the transport detection too narrow for this error (e.g. a `*url.Error` with
an inner that does not satisfy `net.Error`)? Either widen the transport
...
typed before reaching `handleRootError`. Wrap at the boundary (SDK
boundary classifier or a typed constructor) — the bare `error.Error()`
...
- *Read source.* → `errs/doc.go` → `errs/category.go` → `errs/types.go`
  → `errs/predicates.go` → framework classifier → `cmd/root.go`
  `handleRootError`.
