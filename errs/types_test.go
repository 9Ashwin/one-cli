...
func TestPermissionErrorJSONShape(t *testing.T) {
	perm := &errs.PermissionError{
		...
		MissingScopes: []string{"pets:read"},
	}
...
func TestPermissionError_FullChain(t *testing.T) {
	got := errs.NewPermissionError(errs.SubtypeMissingScope,
		"--confirm-send requires scope: %s", "mail:send").
		WithHint("run: onecli auth login --scope %q", "mail:send").
		WithMissingScopes("mail:send").
		WithIdentity("user").
		WithConsoleURL("https://example.com/app/cli_xxx/auth")
	...
	if len(got.MissingScopes) != 1 || got.MissingScopes[0] != "mail:send" {
		t.Errorf("MissingScopes = %v, want [mail:send]", got.MissingScopes)
	}
...
func TestBuilder_ErrorsAsCompat(t *testing.T) {
	...
	if !errors.Is(wrapped, cause) {
		t.Error("errors.Is should resolve to original cause via Unwrap chain")
	}
}
...
func TestBuilder_WireFormat(t *testing.T) {
	e := errs.NewPermissionError(errs.SubtypeMissingScope, "missing scope %s", "pets:create").
		WithCode(12345).
		WithLogID("20260702-0a1b2c3d").
		WithHint("run onecli auth login --scope pets:create").
		WithMissingScopes("pets:create").
		WithIdentity("user").
		WithConsoleURL("https://example.com/app/cli_xxx/auth")
	...
	wantFields := map[string]any{
		...
		"subtype":        "missing_scope",
		"code":           float64(12345),
		"message":        "missing scope pets:create",
		"hint":           "run onecli auth login --scope pets:create",
		"log_id":         "20260702-0a1b2c3d",
		"identity":       "user",
		"console_url":    "https://example.com/app/cli_xxx/auth",
		"missing_scopes": []any{"pets:create"},
	}
...
func TestNewSecurityPolicyError_ChallengeURL(t *testing.T) {
	got := errs.NewSecurityPolicyError(errs.SubtypeChallengeRequired, "verify your device").
		WithCode(21000).
		WithChallengeURL("https://example.com/challenge/xxxxx")
	if got.ChallengeURL == "" {
...
func TestBuilderSetter_DefensiveCopy(t *testing.T) {
	t.Run("WithMissingScopes clones input", func(t *testing.T) {
		...
		err := errs.NewPermissionError(errs.SubtypeMissingScope, "test").
			WithMissingScopes(scopes...)
		scopes[0] = "MUTATED"
		if got := err.MissingScopes[0]; got != "pets:read" {
