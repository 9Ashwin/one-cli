...
const (
	...
	SubtypeFailedPrecondition Subtype = "failed_precondition" // request is valid but the system/resource state is not in the state required to execute; caller must change state (not retry)
)
...
const (
	...
	SubtypeRefreshTokenInvalid Subtype = "refresh_token_invalid" // refresh_token is legacy format, unusable
...
const (
	...
	SubtypeIdentityUnavailable    Subtype = "identity_unavailable"     // requested identity (e.g. --as bot) is not configured
)
...
const (
	...
	SubtypeCrossBrand        Subtype = "cross_brand"        // operation crosses brand boundary (not supported)
	...
	SubtypeQuotaExceeded     Subtype = "quota_exceeded"     // resource quota / collection size limit reached
...
const (
	SubtypeSDKError        Subtype = "sdk_error"        // SDK Do() returned an unexpected error
	SubtypeInvalidResponse Subtype = "invalid_response" // SDK response body not parsable
