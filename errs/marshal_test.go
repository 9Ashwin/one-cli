...
func TestPermissionError_MarshalJSON_HasAllWireFields(t *testing.T) {
	pe := &PermissionError{
		Problem: Problem{
			Category: CategoryAuthorization, Subtype: SubtypeMissingScope, Code: 12345,
...
func TestPermissionError_RequestedGrantedMarshal(t *testing.T) {
	err := NewPermissionError(SubtypeMissingScope, "partial grant").
		WithRequestedScopes("pets:read", "pets:write").
		WithGrantedScopes("pets:read").
		WithMissingScopes("pets:write")
	...
	got := string(b)
	for _, want := range []string{
		`"requested_scopes":["pets:read","pets:write"]`,
		`"granted_scopes":["pets:read"]`,
		`"missing_scopes":["pets:write"]`,
	} {
...
func TestAPIError_MarshalJSON(t *testing.T) {
	ae := &APIError{
		Problem: Problem{Category: CategoryAPI, Subtype: SubtypeRateLimit, Code: 1400, Message: "slow", Retryable: true},
	}
	b, _ := json.Marshal(ae)
	s := string(b)
	for _, want := range []string{
...
// TestProblem_MarshalJSON_Troubleshooter pins the upstream platform
// troubleshooter URL surfacing on the wire under "troubleshooter". Carried
// via Problem so any typed error that embeds it inherits the field.
func TestProblem_MarshalJSON_Troubleshooter(t *testing.T) {
	ae := &APIError{
		Problem: Problem{
			...
			Code:           1400,
			Message:        "x",
			Troubleshooter: "https://example.com/docs/troubleshoot/abc",
		},
	}
	b, _ := json.Marshal(ae)
	s := string(b)
	if !strings.Contains(s, `"troubleshooter":"https://example.com/docs/troubleshoot/abc"`) {
...
// here rather than the struct literal.
