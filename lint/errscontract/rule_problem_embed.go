...
// CheckProblemEmbed enforces that every exported struct whose name ends in
// "Error" embeds the canonical Problem type (bare `Problem` when defined in
// errs, or `errs.Problem` when imported from another package).
func CheckProblemEmbed(path, src string) []Violation {
	...
	ast.Inspect(file, func(n ast.Node) bool {
		...
		// Only enforce on EXPORTED *Error types — unexported helper structs
		// ending in "Error" are internal scratch types, not part of the typed
		// taxonomy.
